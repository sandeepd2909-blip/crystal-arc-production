...
export default async function BlogPost({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const crumbs = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Blog", item: `${SITE_URL}${localePath(LOCALE, "/blog")}` },
    ...
    <>
          ...
          {p.hero ? (
            <Reveal delay={80} immediate>
              <div className="blg-thumb" style={{ aspectRatio: "16/9", marginBottom: "38px" }}>
                ...
                <img src={p.hero} alt={p.heroAlt ?? ""} fetchPriority="high" decoding="async" />
              </div>
            </Reveal>
          ) : null}
      ...
      {work.length ? (
        <section className="xl-work">
          <div className="blg-wrap">
            <div className="eyebrow" style={{ marginBottom: "22px" }}>From our work</div>
              ...
              {work.map((w) => (
                <Link prefetch={false} key={w.slug} href={localePath(LOCALE, `/our-work/${w.slug}`)} className="xl-work-card">
                  ...
                  <div className="xl-work-body">
                    <div className="xl-work-client">{w.client}</div>
                    <div className="xl-work-tag">{w.tagline}</div>
                  </div>
                </Link>
              ))}
            </div>
      ...
      {related.length ? (
        <section style={{ background: "var(--color-s1)", padding: "70px 0 90px" }}>
          <div className="blg-wrap">
            <div className="eyebrow" style={{ marginBottom: "22px" }}>Keep reading</div>
