...
export async function generateMetadata({ params }: { params: Promise<{ topic: string }> }): Promise<Metadata> {
  ...
  const title = `${t.ar} · مجلة كريستال آرك`;
  const description = `${posts.length} مقالة عن ${t.ar} من كريستال آرك، مصنع الدروع وهدايا الشركات في دبي.`;
  return {
    title,
    description,
    alternates: { canonical: url },
    openGraph: { title, description, url, type: "website", images: ogImages(posts[0]?.hero, t.ar) },
  };
}
...
export default async function TopicArchive({ params }: { params: Promise<{ topic: string }> }) {
  ...
  const schema = {
    ...
    isPartOf: { "@type": "Blog", name: "مجلة كريستال آرك", url: `${SITE_URL}${localePath(LOCALE, "/blog")}` },
    ...
    <>
      ...
      <section style={{ padding: "clamp(120px, 16vh, 170px) 0 32px" }}>
        <div className="blg-wrap">
          <Reveal immediate>
            <nav aria-label="مسار التنقل" className="blg-crumb">
              <Link prefetch={false} href={localePath(LOCALE, "/blog")}>المجلة</Link>
              <span> › </span>
              <span>{t.ar}</span>
            </nav>
            <h1 className="blg-h1">{t.ar}</h1>
            <p className="blg-sub">{posts.length} مقالة</p>
          </Reveal>
        </div>
      </section>
